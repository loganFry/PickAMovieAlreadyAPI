...
exports.removeMovie = async function(req, res, next){
    ...
    try {
        var updatedPoll = await PollService.removeMovie(pollId, movieId);
        return res.status(200).json({status: 200, data: updatedPoll, message: "Successfully removed movie from poll"})
    } catch(e) {
        return res.status(400).json({status: 400, message: e.message})
    }
}
...
exports.voteForMovie = async function(req, res, next){
    var pollId = req.params.id;
    var movieId = req.body.movieId;
    var oldMovieId = req.body.oldMovieId;

    try {
        var updatedPoll = await PollService.voteForMovie(pollId, movieId, oldMovieId);
